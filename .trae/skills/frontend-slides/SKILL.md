...
Identify the effect boundaries (which CSS + JS + HTML belongs to this effect)

Extract parameterizable values (colors → `var(--accent)`, counts → comments)

Write a new `## EFFECT-XXX` entry in `EFFECT_LIBRARY.md`:
...
Confirm to user: *"Saved as EFFECT-006. I'll suggest it when I see similar content in the future."*
...
If there is no `index.html`, the root rewrite to a named file is required — Vercel will 404 without it

Files with spaces in filenames (e.g., `social media.jpg`) are valid in Vercel static hosting

.mov and large media files: Vercel has a 100MB single-file limit

All media files must be in the same directory as the HTML files to be included in the deploy

Re-deploy command: `npx vercel --prod --yes`
